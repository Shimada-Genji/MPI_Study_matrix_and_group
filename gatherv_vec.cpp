...
//gatherv
  ...
  for(int j=0;j<10;j++){
    gatherv[j]=rank;//gatherv收集的多个值都是本身的进程号
  }
  MPI_Type_vector(rank+1,1,1,MPI_INT,&vec);//每个进程发送的整数数目不同，定义一个新的数据类型，每个进程都发送一个向量。
  MPI_Type_commit(&vec);
//gatherv的收集方法
...
//各进程已准备好被收集的数据,申请收集结果存放的空间。
  ...
  MPI_Gatherv(gatherv,10,MPI_INT,gatherv_recv,every_proc_count,displs,MPI_INT,0,MPI_COMM_WORLD);//
  //测试结果
  if(rank==0)
  {
    for(int j=0;j<proc_size;j++){
